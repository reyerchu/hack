...
export default function UserPage() {
  ...
  async function fetchAllUsers() {
    ...
    const sortedByDate = hackersOnly.sort((a, b) => {
      ...
      const timeA =
        a.timestamp || ((a as any).createdAt?._seconds ? (a as any).createdAt._seconds * 1000 : 0);
      const timeB =
        b.timestamp || ((b as any).createdAt?._seconds ? (b as any).createdAt._seconds * 1000 : 0);
    ...
    setFilteredUsers((prev) =>
      [...prev].sort((a, b) => {
        ...
        if (field === 'registeredDate') {
          ...
          const timeA =
            a.timestamp ||
            ((a as any).createdAt?._seconds ? (a as any).createdAt._seconds * 1000 : 0);
          const timeB =
            b.timestamp ||
            ((b as any).createdAt?._seconds ? (b as any).createdAt._seconds * 1000 : 0);
          return timeB - timeA; // Descending order (newest first)
        }
  ...
  const exportToCSV = () => {
    ...
    const rows = filteredUsers.map((userData) => {
      ...
      if (userData.timestamp) {
        ...
        registeredDate = new Date((userData as any).createdAt._seconds * 1000).toLocaleDateString(
          'zh-TW',
          {
            year: 'numeric',
            month: '2-digit',
