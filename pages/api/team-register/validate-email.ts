...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!userSnapshot.empty) {
      ...
      const name =
        `${firstName} ${lastName}`.trim() || userData.user?.nickname || userData.nickname || '';
    ...
    for (const doc of allRegistrations.docs) {
      const data = doc.data();
      const emails = [data.email, data.user?.email, data.user?.preferredEmail]
        .filter((e) => e)
      ...
      if (emails.includes(normalizedEmail)) {
        ...
        const name =
          `${firstName} ${lastName}`.trim() || data.user?.nickname || data.nickname || '';
    ...
    for (const doc of allUsers.docs) {
      const data = doc.data();
      const emails = [data.email, data.preferredEmail]
        .filter((e) => e)
        .map((e) => e.toLowerCase().trim());
    ...
    console.log(
      '[ValidateEmail] ❌ Email not found even after full collection scan:',
      normalizedEmail,
    );
    return res.status(200).json({
      isValid: false,
