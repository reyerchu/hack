...
export default async function handler(
  ...
  try {
    ...
    if (!trackDoc.exists) {
      ...
      console.log('[API /tracks/[trackId]] Method 2 (extended-sponsors) - exists:', trackDoc.exists);
    }
    ...
    if (trackData.type && trackData.type !== 'track') {
      console.log('[API /tracks/[trackId]] Not a track:', trackId, 'type:', trackData.type, 'name:', trackData.name || trackData.title);
      return res.status(404).json({ error: 'Not a track', actualType: trackData.type });
    }
    
    console.log('[API /tracks/[trackId]] Found track - name:', trackData.name, 'trackId field:', trackData.trackId);
        ...
        if (Array.isArray(challenge.prizes)) {
          challenge.prizes.forEach((prize: any) => {
            if (typeof prize === 'object' && prize.amount) {
              ...
              const amountInUSD = prize.currency === 'TWD' 
                ? prize.amount / 30 
                : prize.amount;
