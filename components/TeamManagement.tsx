...
const TeamManagement: React.FC = () => {
  ...
  // PDF upload states
  const [pdfUploading, setPdfUploading] = useState<{ [key: string]: boolean }>({});
  const [pdfDeleting, setPdfDeleting] = useState<{ [key: string]: boolean }>({});
  const [pdfMessage, setPdfMessage] = useState<{ [key: string]: string }>({});
  ...
  // Handle PDF upload
  const handlePdfUpload = async (teamId: string, teamName: string, file: File) => {
    if (!user?.token) return;

    // Validate file type
    if (file.type !== 'application/pdf') {
      setPdfMessage({ ...pdfMessage, [teamId]: '請上傳 PDF 文件' });
      return;
    }

    // Validate file size (max 10MB)
    if (file.size > 10 * 1024 * 1024) {
      setPdfMessage({ ...pdfMessage, [teamId]: '文件大小不能超過 10MB' });
      return;
    }

    try {
      setPdfUploading({ ...pdfUploading, [teamId]: true });
      setPdfMessage({ ...pdfMessage, [teamId]: '上傳中...' });
      ...
      if (!response.ok || data.error) {
        setPdfMessage({ ...pdfMessage, [teamId]: data.error || '上傳失敗' });
        return;
      }

      setPdfMessage({ ...pdfMessage, [teamId]: '上傳成功！已發送通知郵件給管理員。' });
      ...
      setTimeout(() => {
        setPdfMessage({ ...pdfMessage, [teamId]: '' });
      }, 3000);
    } catch (error: any) {
      console.error('[TeamManagement] PDF upload error:', error);
      setPdfMessage({ ...pdfMessage, [teamId]: '上傳失敗：' + error.message });
    } finally {
      setPdfUploading({ ...pdfUploading, [teamId]: false });
    }
  };

  // Handle PDF delete
  const handlePdfDelete = async (teamId: string) => {
    if (!user?.token) return;
    ...
    try {
      setPdfDeleting({ ...pdfDeleting, [teamId]: true });
      ...
      setPdfMessage({ ...pdfMessage, [teamId]: 'PDF 已刪除' });
      ...
      console.error('[TeamManagement] PDF delete error:', error);
      alert('刪除失敗：' + error.message);
    } finally {
      setPdfDeleting({ ...pdfDeleting, [teamId]: false });
    }
  };
    ...
    <div className="space-y-4">
      {teams.map((team) => (
        ...
        >
          ...
          {/* PDF Submission Section - Only for users with edit rights */}
          {team.canEdit && (
            <div className="mt-4 pt-4 border-t" style={{ borderColor: '#e5e7eb' }}>
              <div className="text-sm font-medium text-gray-700 mb-3">提交 PDF 文件：</div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-green-700">
                          文件：{team.submittedPdf.fileName}
                        </div>
                        <div className="text-xs text-green-600">
                          上傳時間：{formatDate(team.submittedPdf.uploadedAt)}
                        </div>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <a
                        href={team.submittedPdf.fileUrl}
                        target="_blank"
                      ...
                      <button
                        onClick={() => handlePdfDelete(team.id)}
                        disabled={pdfDeleting[team.id]}
                        className="px-4 py-2 rounded-lg font-medium transition-colors text-sm"
                        style={{
                          backgroundColor: pdfDeleting[team.id] ? '#9ca3af' : '#dc2626',
                          color: 'white',
                          cursor: pdfDeleting[team.id] ? 'not-allowed' : 'pointer',
                        }}
                        onMouseEnter={(e) => {
                          if (!pdfDeleting[team.id])
                            e.currentTarget.style.backgroundColor = '#b91c1c';
                        }}
                        onMouseLeave={(e) => {
                          if (!pdfDeleting[team.id])
                ...
                <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg p-4">
                  <div className="text-center">
                    ...
                    <div className="mt-2">
                      ...
                      >
                        {pdfUploading[team.id] ? '上傳中...' : '選擇 PDF 文件'}
                      </label>
                      <input
                        id={`pdf-upload-${team.id}`}
                        type="file"
                        accept=".pdf"
                        className="hidden"
                        disabled={pdfUploading[team.id]}
                        onChange={(e) => {
                          const file = e.target.files?.[0];
                          if (file) {
                            handlePdfUpload(team.id, team.teamName, file);
                          }
                          e.target.value = '';
                        }}
                      />
                    </div>
                    <p className="text-xs text-gray-500 mt-2">僅接受 PDF 格式，最大 10MB</p>
                    <p className="text-xs text-gray-500 mt-1">提交後將發送通知郵件給管理員</p>
                  </div>
                </div>
              )}
              ...
              {pdfMessage[team.id] && (
                <div
                  ...
                  style={{
                    backgroundColor: pdfMessage[team.id].includes('成功')
                      ? '#d1fae5'
                      : pdfMessage[team.id].includes('刪除')
                      ? '#fee2e2'
                      : '#fef3c7',
                    color: pdfMessage[team.id].includes('成功')
                      ? '#065f46'
                      : pdfMessage[team.id].includes('刪除')
                      ? '#991b1b'
                      : '#92400e',
