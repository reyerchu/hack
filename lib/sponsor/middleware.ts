...
import { checkSponsorPermission, checkTrackAccess, getUserSponsorRole } from './permissions';
...
export async function requireAuth(req: NextApiRequest, res: NextApiResponse): Promise<boolean> {
  try {
    ...
    const userEmail =
      userData?.email || nestedUser?.preferredEmail || userData?.preferredEmail || email; // 使用之前從 token 獲取的 email
