...
async function checkAndFixSponsorAccess() {
  ...
  try {
    ...
    // 先尝试 registrations
    let userSnapshot = await db.collection('registrations')
    ...
    if (userSnapshot.empty) {
      ...
      userSnapshot = await db.collection('registrations')
    ...
    if (userSnapshot.empty) {
      ...
      for (const doc of allUsers.docs) {
        const data = doc.data();
        const email = data.email || data.preferredEmail || data.user?.email || data.user?.preferredEmail;
    ...
    const mappingSnapshot = await db.collection('sponsor-user-mappings')
      .where('userId', '==', userId)
    ...
    // 6. 测试访问权限
    console.log('\n6️⃣ 测试访问权限...');
    const testMappingSnapshot = await db.collection('sponsor-user-mappings')
      .where('userId', '==', userId)
      .get();
    ...
    const testTrackSnapshot = await db.collection('tracks')
